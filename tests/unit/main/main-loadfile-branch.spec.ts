...
describe('main.ts loadFile fallback branch', () => {
  it('falls back to loadFile when dev server not available', async () => {
    ...
    const electronMock: any = {
      ...
      BrowserWindow: class {
        webContents: any = { send: vi.fn(), isDevToolsOpened: vi.fn(() => false), openDevTools: vi.fn(), closeDevTools: vi.fn() };
        constructor() {
          // simulate dev server not available
          (this as any).loadURL = async (_u: string) => { throw new Error('dev not available'); };
          (this as any).loadFile = async (_f: string) => { fileLoaded.v = true; };
        }
