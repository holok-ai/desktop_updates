...
import { ApiRetry, DEFAULT_RETRY_CONFIG, RetryExhaustedError } from '../../../src-electron/utils/apiretry.js';
...
describe('API Retry Utility', () => {
  ...
  describe('ApiRetry.execute - Failure Cases', () => {
    it('should throw RetryExhaustedError after max attempts', async () => {
      ...
      await expect(
        ApiRetry.execute(fn, DEFAULT_RETRY_CONFIG, 'test')
  ...
  describe('ApiRetry.execute - Retry Configuration', () => {
    it('should respect custom maxAttempts', async () => {
      ...
      await expect(
        ApiRetry.execute(fn, customConfig, 'test')
      ).rejects.toThrow(RetryExhaustedError);
