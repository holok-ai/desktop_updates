 ...
 */
export function getMessagesToHideAfterForkPoint(
  messages: Message[],
  onlyHideBeforeVariation: boolean = false,
): Set<string> {
  ...
  for (const forkBranchId of forkPoints) {
    ...
    for (const msg of messages) {
      ...
      const isOriginalBranchPath =
        msgParts.length === 3 && msgParts[1] === '0' && msgParts[2] === '0';
  ...
  const topLevelBranchIds = messages
    ...
    .map((bid) => {
      ...
      const num = Number.parseInt(
        firstPart !== undefined && firstPart.length > 0 ? firstPart : '0',
        10,
      );
 ...
 */
export function getBranchBoxes(
  messages: Message[],
  forkBranchId: string,
): Array<{
  branchId: string;
...
export function buildContextFromSelectedBranches(
  ...
  for (const forkBranchId of forkPoints) {
    ...
    const isCurrentVariationForkPoint =
      currentVariationForkPointRow !== null && currentVariationForkPointRow === forkPointRowNum;
                  ...
                  if (msgRowNum < nextForkPointRowNum) {
                    ...
                    if (selectedBranchAtThisFork) {
                      ...
                      for (const msg2 of messages) {
                        ...
                        const isMsg2OriginalPath =
                          msg2Parts.length === 3 && msg2Parts[1] === '0' && msg2Parts[2] === '0';
  ...
  const firstForkPointIndex = messages.findIndex(
    (m) => m.branchId === firstForkPoint && m.role === 'user',
  );

  // Get all messages before the first fork point
  const messagesBeforeFirstFork =
    firstForkPointIndex >= 0 ? messages.slice(0, firstForkPointIndex) : [];
  ...
  for (const msg of messages.slice(firstForkPointIndex >= 0 ? firstForkPointIndex : 0)) {
    ...
    if (currentVariationForkPointMessage?.id === msg.id) {
    ...
    if (
      currentVariationForkPointRow !== null &&
      isOriginalBranchPath &&
      msgRowNum > currentVariationForkPointRow
    ) {
    ...
    if (isOriginalBranchPath) {
      ...
      const comesAfterAnySelectedBranch =
        selectedBranchIds.length > 0 &&
        selectedBranchIds.some((branchId) => {
          const branchRowNum = getRowNumber(branchId);
          return msgRowNum > branchRowNum;
        });
    ...
    if (
      selectedBranchInRow !== null &&
      selectedBranchInRow !== undefined &&
      selectedBranchInRow !== ''
    ) {
      ...
      if (selectedBranchMessages.has(msg.id)) {
        ...
        normalizedMsgId === selectedBranchInRow ||
        normalizedMsgId.startsWith(`${selectedBranchInRow}.`)
      ) {
