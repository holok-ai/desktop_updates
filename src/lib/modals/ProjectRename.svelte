<script lang="ts">
  ...
  let { projectId, currentTitle }: { projectId: string; currentTitle: string } = $props();

  const dispatch = createEventDispatcher<{
    confirm: { projectId: string; newTitle: string };
    cancel: void;
  }>();
  ...
  async function handleSubmit(e: Event): Promise<void> {
    ...
    isSubmitting = true;
    dispatch('confirm', { projectId, newTitle: newTitle.trim() });
...
>
  ...
  >
    <div class="dialog-header">
      <h2 id="rename-dialog-title">Rename Project</h2>
