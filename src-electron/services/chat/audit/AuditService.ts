...
export class AuditService {
    ...
    public logChatAudit(auditData: ChatAuditData): void {
        ...
        if (this.config.logToConsole) {
            console.log(`[AUDIT] Chat completed: ${filteredData.provider}/${filteredData.model} (${filteredData.totalDuration}ms)`, filteredData);
        }
    ...
    private async sendToServer(auditData: ChatAuditData, endpoint: string): Promise<void> {
        try {
            ...
            if (!response.ok) {
                console.error(`Failed to send audit data to server: ${response.status} ${response.statusText}`);
            }
        } catch (error) {
