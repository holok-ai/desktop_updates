...
Multiple desktop users connected to the same Holokai project must stay in sync in real time — new chat messages, project, project member, instruction and file changes should propagate to every "subscribed" member without requiring a manual refresh. This document specifies the requirements, message exchange protocol, new Moku API surface, and the rationale for choosing SSE + REST over WebSockets.

Moku API will provide a "Moku API SSE Service" that handles the server-side connection, maintains subscribed users, and distributes change events. Desktop users subscribe by Project id, and receive change events from the SSE Service only for the project id they are "in". Desktop project loading will load latest project data when a user loads it from the Project List (the Desktop project-repository will be modified to not cache from load, but to check "lastupdated".)
...
- **Desktop Subscribe to Project Changes** — When a user navigates to a project route, the desktop will call a subscribe Moku endpoint (opens a persistent SSE connection to Moku) for the current project. Project change events are then sent mrom Moku through SSE to the Desktop. Moku will broadcast project change events to users who have subscribed.
...
- **Desktop New Prompt and Response Events** — Two distinct events to capture: 1) once a user submits a new prompt, a `prompt-created` event is sent to all subscribed users to show the new prompt; 2) once the response is complete, a `response-created` event is sent to all subscribed users to show the nre response. Two design options for how Moku learns of these events:
