...
const te = class te {
  ...
  static async handleToolLoop(e, n, o, i, r, a, u, d = !1) {
    ...
    for (let m = 0; m < te.MAX_TOOL_ITERATIONS; m++) {
      ...
      const g = await e.makeRequest(n, f, i, p, d);
      if (!d || !h) {
  ...
  static toOpenAIRequest(e, n) {
    if (e) {
      ...
      if (n.system) {
        ...
        o = [{ role: 'system', content: n.system }, ...a.map((u) => this.mapMessage(u, !1))];
      } else o = n.messages.map((a) => this.mapMessage(a, !1));
      const i = {
        model: n.model,
        messages: o,
        stream: n.streaming !== !1,
      ...
      const r = o.length > 0 ? JSON.stringify(o[0]).substring(0, 50) : 'none';
      return (
        console.log(
          `[OpenAIConverter] Chat Completions API - ${o.length} messages, first: ${r}...`,
        ),
        i
      );
    }
...
class ye {
  ...
  async chatStreaming(e, n, o) {
    ...
    try {
      ...
      for await (const p of u) {
        if (
          (d++, (r = p.response) != null && r.id && (m = p.response.id), p.type === 'text_delta')
        ) {
  ...
  async chatNonStreaming(e, n, o) {
    ...
    try {
      ...
      const p =
        ((d = f.output) == null ? void 0 : d.filter((y) => y.type === 'function_call')) || [];
      if (p.length > 0)
        return (
      ...
      const h = ((c = f.output) == null ? void 0 : c.filter((y) => y.type === 'message')) || [],
        g = [];
      for (const y of h)
        if (y.content)
  ...
  async makeStreamingRequest(e, n, o, i) {
    try {
      const r = Date.now(),
        ...
        d = Oe.toOpenAIRequest(u, a),
        c = await this.chatHandler.chatStreaming(d, e, this.onTokenReceived),
        f = Date.now() - r;
      return c;
    } catch (r) {
      throw (
  ...
  async makeNonStreamingRequest(e, n, o, i) {
    try {
      const r = Date.now(),
        ...
        u = this.chatHandler instanceof ye,
        d = Oe.toOpenAIRequest(u, a),
        c = await this.chatHandler.chatNonStreaming(d, e, this.onTokenReceived),
        f = Date.now() - r;
      return c;
    } catch (r) {
      throw (
  ...
  async chatStreaming(e, n, o) {
    ...
    try {
      ...
      for await (const h of d) {
        ...
        if (
          (T != null && T.content && ((c += T.content), o && o(T.content)),
          T != null && T.tool_calls)
        )
  ...
  async chatNonStreaming(e, n, o) {
    ...
    try {
      ...
      if (
        ...
        throw (
          console.error('[ChatCompletionsHandler] Invalid response structure:', d),
          new Error('Response missing choices array')
        );
      const c =
        ((u = (a = d.choices[0]) == null ? void 0 : a.message) == null ? void 0 : u.content) || '';
      return (c && o && o(c), d);
    } catch (d) {
  ...
  for (let r = 0; r < e.length - 1; r++) {
    ...
    if (a.endsWith('[]')) {
      ...
      if (Array.isArray(t[u])) {
        ...
        else for (const c of d) l(c, e.slice(r + 1), n);
      }
      return;
...
function $e(t, e, n, o, i) {
  ...
  if (r.endsWith('[]')) {
    const a = r.slice(0, -2),
      u = t;
    if (a in u && Array.isArray(u[a])) for (const d of u[a]) $e(d, e, n, o + 1, i);
  } else if (r === '*') {
    if (typeof t == 'object' && t !== null && !Array.isArray(t)) {
      ...
      for (const [c, f] of Object.entries(d)) {
        const m = [];
        for (const p of n.slice(o)) p === '*' ? m.push(c) : m.push(p);
        l(a, m, f);
      }
...
class Fi {
  ...
  get data() {
    ...
    for (const a of (o =
      ...
      ? o
      : []) {
      for (const [u, d] of Object.entries(a)) u !== 'inlineData' && d !== null && r.push(u);
  ...
  for (const [a, u] of Object.entries(t))
    if (u != null)
      if (a == 'type') {
        ...
        const d = {};
        for (const [c, f] of Object.entries(u)) d[c] = ue(f);
        e[a] = d;
      } else {
...
class Vs extends X {
  ...
  async createInternal(e) {
    ...
    if (this.apiClient.isVertexAI()) {
      ...
      return (
        (u = v('batchPredictionJobs', c._url)),
        (d = c._query),
        delete c._url,
        delete c._query,
        (a = this.apiClient
      ...
      return (
        (u = v('{model}:batchGenerateContent', c._url)),
        (d = c._query),
        delete c._url,
        delete c._query,
        (a = this.apiClient
  ...
  async get(e) {
    ...
    if (this.apiClient.isVertexAI()) {
      ...
      return (
        (u = v('batchPredictionJobs/{name}', c._url)),
        (d = c._query),
        delete c._url,
        delete c._query,
        (a = this.apiClient
  ...
  async cancel(e) {
    ...
    if (this.apiClient.isVertexAI()) {
      ...
      ((a = v('batchPredictionJobs/{name}:cancel', d._url)),
        (u = d._query),
        delete d._url,
        delete d._query,
      ...
      ((a = v('batches/{name}:cancel', d._url)),
        (u = d._query),
        delete d._url,
        delete d._query,
        await this.apiClient.request({
  ...
  async sendMessage(e) {
    ...
    return (
      (this.sendPromise = (async () => {
        ...
        const d = await i,
          c =
            (a = (r = d.candidates) === null || r === void 0 ? void 0 : r[0]) === null ||
            a === void 0
              ? void 0
              : a.content,
...
function Kr(t) {
  ...
  const u = s(t, ['goAway']);
  u != null && l(e, ['goAway'], u);
  const d = s(t, ['sessionResumptionUpdate']);
  d != null && l(e, ['sessionResumptionUpdate'], d);
  const c = s(t, ['voiceActivityDetectionSignal']);
...
function Zr(t) {
  ...
  const d = s(t, ['promptTokensDetails']);
  if (d != null) {
    let h = d;
    (Array.isArray(h) && (h = h.map((g) => g)), l(e, ['promptTokensDetails'], h));
  }
  const c = s(t, ['cacheTokensDetails']);
  if (c != null) {
    let h = c;
...
function xl(t, e) {
  ...
  const d = s(t, ['responseJsonSchema']);
...
function kl(t, e, n) {
  ...
  const c = s(t, ['includeSafetyAttributes']);
...
function Qe(t, e) {
  ...
  const d = s(t, ['inputTokenLimit']);
  d != null && l(n, ['inputTokenLimit'], d);
  const c = s(t, ['outputTokenLimit']);
...
function Ea(t, e) {
  ...
  const d = s(t, ['styleImageConfig']);
  d != null && l(n, ['styleImageConfig'], d);
  const c = s(t, ['subjectImageConfig']);
...
function va(t, e, n) {
  ...
  const d = s(t, ['binaryColorThreshold']);
...
function Fa(t, e, n) {
  ...
  const d = s(t, ['outputMimeType']);
  e !== void 0 && d != null && l(e, ['parameters', 'outputOptions', 'mimeType'], d);
  const c = s(t, ['outputCompressionQuality']);
...
`,
  ...
  async uploadFile(e, n) {
    ...
    const d = {
      ...
      f = v('upload/v1beta/files', d._url),
...
class Tu {
  ...
  async connect(e) {
    ...
    const i = this.apiClient.getWebsocketBaseUrl(),
      ...
      d = `${i}/ws/google.ai.generativelanguage.${r}.GenerativeService.BidiGenerateMusic?key=${u}`;
    let c = () => {};
  ...
  async connect(e) {
    ...
    if (this.apiClient.isVertexAI()) {
      ...
      (P != null &&
        ...
        (U = 'access_token')),
        (f = `${d}/ws/google.ai.generativelanguage.${c}.GenerativeService.${N}?${U}=${P}`));
    }
    let h = () => {};
...
class Nu extends X {
      ...
      (this.generateContent = async (n) => {
        ...
        for (
          ...
          (g++,
            ...
            (d.contents = B(d.contents)),
            d.contents.push(T),
            d.contents.push(m),
            Jn(d.config) && (p.push(T), p.push(m)));
        }
          ...
          if (o != null && o.generatedImages)
            for (const d of o.generatedImages)
              ...
              ((i = d == null ? void 0 : d.safetyAttributes) === null || i === void 0
                ...
                ? (r = d == null ? void 0 : d.safetyAttributes)
                : a.push(d);
          let u;
          return (
            r
  ...
  async processParamsMaybeAddMcpUsage(e) {
    ...
    if (!r) return e;
    const a = await Promise.all(r.map(async (d) => (de(d) ? await d.tool() : d))),
      u = {
        model: e.model,
        contents: e.contents,
    ...
    return (function (c, f, m) {
      return O(this, arguments, function* () {
        for (var p, h, g, T, y, _; u < r; ) {
          a && (u++, (a = !1));
          const C = yield M(c.processParamsMaybeAddMcpUsage(m)),
            A = yield M(c.generateContentStreamInternal(C)),
            R = [],
            x = [];
          try {
            for (
  ...
  async generateContentInternal(e) {
    ...
    if (this.apiClient.isVertexAI()) {
      ...
      return (
        (u = v('{model}:generateContent', c._url)),
        (d = c._query),
        delete c._url,
        delete c._query,
        (a = this.apiClient
  ...
  async generateContentStreamInternal(e) {
    ...
    if (this.apiClient.isVertexAI()) {
      const c = Fn(this.apiClient, e);
      return (
        (u = v('{model}:streamGenerateContent?alt=sse', c._url)),
        (d = c._query),
        delete c._url,
        delete c._query,
  ...
  async embedContent(e) {
    ...
    if (this.apiClient.isVertexAI()) {
      ...
      return (
        (u = v('{model}:batchEmbedContents', c._url)),
        (d = c._query),
        delete c._url,
        delete c._query,
        (a = this.apiClient
  ...
  async generateVideosInternal(e) {
    ...
    if (this.apiClient.isVertexAI()) {
      ...
      return (
        (u = v('{model}:predictLongRunning', c._url)),
        (d = c._query),
        delete c._url,
        delete c._query,
        (a = this.apiClient
...
function ku(t, e, n) {
  ...
  const d = s(e, ['lockAdditionalFields']);
...
class He {
  ...
  constructor(e) {
    var n,
      ...
      d,
      {
        baseURL: c = Be('GEMINI_NEXT_GEN_API_BASE_URL'),
    ...
    ((this.logLevel = g),
      ...
      (this.fetch = (d = h.fetch) !== null && d !== void 0 ? d : gd()),
      (this.encoder = _d),
      (this._options = h),
      (this.apiKey = f),
      (this.apiVersion = m),
  ...
  async makeRequest(e, n, o) {
    ...
    const T = new AbortController(),
      y = await this.fetchWithTimeout(f, c, m, T).catch(tt),
      _ = Date.now();
...
function qd(t, e, n) {
  ...
  if (
    (e !== void 0 && d != null && l(e, ['tuningTask', 'hyperparameters', 'learningRate'], d),
    s(t, ['labels']) !== void 0)
  )
    throw new Error('labels parameter is not supported in Gemini API.');
...
function Wo(t, e) {
  ...
  const d = s(t, ['tuningTask', 'completeTime']);
...
class se {
  ...
  static toGeminiRequest(e, n, o) {
    const r = e.messages.filter((d) => d.role !== 'system').map((d) => this.mapMessage(d)),
      a = {
        model: n,
        contents: r,
      };
  ...
  async chat(e, n) {
    ...
    try {
      if (((e.streaming = !1), e.streaming !== !1)) {
        ...
        let g = null;
        const T =
          (f = (c = (d = m.candidates) == null ? void 0 : d[0]) == null ? void 0 : c.content) ==
          null
            ? void 0
            : f.parts;
        if (T)
    ...
    for (; c < Z.MAX_TOOL_ITERATIONS; ) {
      ...
      try {
        if (a) {
          ...
          if (R) {
            (console.log('[GeminiChatProvider] Retrying with non-streaming mode', { iteration: c }),
...
const ne = class ne {
  ...
  logChatAudit(e) {
    ...
    if (this.config.logToConsole) {
      const i = {
        ...n,
        promptText: `[length: ${((o = n.promptText) == null ? void 0 : o.length) || 0}]`,
      };
      console.log(
        `[AUDIT] Chat completed: ${i.provider}/${i.model} (${i.totalDuration}ms)`,
        JSON.stringify(i),
      );
    }
