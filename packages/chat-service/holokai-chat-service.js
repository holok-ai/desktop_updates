...
const te = class te {
  ...
  static async handleToolLoop(e, n, o, i, r, a, u, d = !1) {
    ...
    for (let m = 0; m < te.MAX_TOOL_ITERATIONS; m++) {
      ...
      h ? e.setTokenCallback(u) : console.log("[ChatProviderUtils] handleToolLoop - Handler does not support streaming callback");
      ...
      }), console.log(`[ChatProviderUtils] Messages array (${f.length} messages):`), f.forEach((E, C) => {
        const A = E.content && E.content.length > 50 ? E.content.substring(0, 50) + "..." : E.content, R = [];
        E.tool_calls && R.push(`tool_calls: ${E.tool_calls.length}`), E.tool_call_id && R.push(`tool_call_id: ${E.tool_call_id}`);
  ...
  static toOpenAIRequest(e, n) {
    if (e) {
      ...
      if (n.system) {
        ...
        o = [
          { role: "system", content: n.system },
          ...a.map((u) => this.mapMessage(u, !1))
        ];
      } else
        o = n.messages.map((a) => this.mapMessage(a, !1));
      const i = {
        model: n.model,
        messages: o,
        stream: n.streaming !== !1
      };
      n.temperature !== void 0 && (i.temperature = n.temperature), n.maxTokens !== void 0 && (i.max_completion_tokens = n.maxTokens), n.topP !== void 0 && (i.top_p = n.topP), n.frequencyPenalty !== void 0 && (i.frequency_penalty = n.frequencyPenalty), n.presencePenalty !== void 0 && (i.presence_penalty = n.presencePenalty), n.stop !== void 0 && (i.stop = n.stop), n.responseFormat !== void 0 && (i.response_format = n.responseFormat), n.thread_id !== void 0 && (i.thread_id = n.thread_id), n.branch_id !== void 0 && (i.branch_id = n.branch_id), n.tools !== void 0 && (i.tools = n.tools), n.responseFormat !== void 0 && (i.response_format = {
        type: "json_schema",
        json_schema: {
          name: "schema_name",
      ...
      return console.log(`[OpenAIConverter] Chat Completions API - ${o.length} messages, first: ${r}...`), i;
    }
  }
}
class ye {
...
class eo {
  ...
  async chat(e, n) {
    ...
    if (this.chatHandler = ni.create(this.client, this.endpointType), console.log(`OpenAI Chat Provider (${o ? "response" : " chat"}) | model: ${e.model || this.defaultModel} | streaming: ${e.streaming !== !1} | hasTools: ${this.tools.length > 0} | toolCount: ${this.tools.length}`), this.tools.length > 0 && this.onToolUse) {
...
class oi {
  ...
  extractTextContent(e) {
    return e.content.filter((o) => o.type === "text").map((o) => o.text).join("") || null;
  }
  formatToolResults(e, n) {
    return e.map((o, i) => {
      const r = n[i];
...
function ue(t) {
  ...
  const r = t.anyOf;
  r != null && r.length == 2 && (r[0].type === "null" ? (e.nullable = !0, t = r[1]) : r[1].type === "null" && (e.nullable = !0, t = r[0])), t.type instanceof Array && Vi(t.type, e);
...
class Cr {
  ...
  async sendMessage(e) {
    ...
    const o = V(e.message), i = this.modelsModule.generateContent({
      model: this.model,
  ...
  async sendMessageStream(e) {
    var n;
    await this.sendPromise;
    const o = V(e.message), i = this.modelsModule.generateContentStream({
      model: this.model,
      contents: this.getHistory(!0).concat(o),
      config: (n = e.config) !== null && n !== void 0 ? n : this.config
    });
...
class cu {
  ...
  async unaryApiCall(e, n, o) {
    return this.apiCall(e.toString(), Object.assign(Object.assign({}, n), { method: o })).then(async (i) => (await Hn(i), new Ye(i))).catch((i) => {
...
class vu {
  ...
  async connect(e) {
    ...
    return this.apiClient.isVertexAI() ? C = Br(this.apiClient, x) : C = qr(this.apiClient, x), delete C.config, S.send(JSON.stringify(C)), new Ru(S, this.apiClient);
  }
          ...
          try {
            for (var I = !0, S = (h = void 0, $(A)), E; E = yield M(S.next()), p = E.done, !p; I = !0) {
              ...
              if (yield yield M(P), P.candidates && (!((y = P.candidates[0]) === null || y === void 0) && y.content)) {
                ...
                for (const N of (_ = P.candidates[0].content.parts) !== null && _ !== void 0 ? _ : [])
        ...
        return O(this, arguments, function* () {
          ...
          try {
            for (var y = !0, _ = $(m), I; I = yield M(_.next()), p = I.done, !p; y = !0) {
              T = I.value, y = !1;
              const S = T, E = Vn(yield M(S.json()), e);
              E.sdkHttpResponse = {
                headers: S.headers
              };
              const C = new he();
      ...
      return u = v("{model}:streamGenerateContent?alt=sse", c._url), d = c._query, delete c._url, delete c._query, a = this.apiClient.requestStream({
        ...
        return O(this, arguments, function* () {
          ...
          try {
            for (var y = !0, _ = $(m), I; I = yield M(_.next()), p = I.done, !p; y = !0) {
              T = I.value, y = !1;
              const S = T, E = Gn(yield M(S.json()), e);
              E.sdkHttpResponse = {
                headers: S.headers
              };
              const C = new he();
...
const De = {
  ...
  if (t) {
    if (ad(De, t))
      return t;
    H(n).warn(`${e} was set to ${JSON.stringify(t)}, expected one of ${JSON.stringify(Object.keys(De))}`);
  }
};
function Te() {
}
...
async function Ld(t, e) {
  const { response: n, requestLogID: o, retryOfRequestLogID: i, startTime: r } = e, a = await (async () => {
    var u;
    if (e.options.stream)
      return H(t).debug("response", n.status, n.url, n.headers, n.body), e.options.__streamClass ? e.options.__streamClass.fromSSEResponse(n, e.controller, t) : ae.fromSSEResponse(n, e.controller, t);
    if (n.status === 204)
      return null;
...
class He {
  ...
  async makeRequest(e, n, o) {
    ...
    if (y instanceof globalThis.Error) {
      ...
      if (n)
        return H(this).info(`[${p}] connection ${E ? "timed out" : "failed"} - ${S}`), H(this).debug(`[${p}] connection ${E ? "timed out" : "failed"} (${S})`, ee({
          retryOfRequestLogID: o,
          url: f,
          durationMs: _ - g,
          message: y.message
    ...
    if (!y.ok) {
      ...
      if (n && S) {
        ...
        return await Td(y.body), H(this).info(`${I} - ${P}`), H(this).debug(`[${p}] response error (${P})`, ee({
          retryOfRequestLogID: o,
          url: y.url,
          status: y.status,
          headers: y.headers,
    ...
    return H(this).info(I), H(this).debug(`[${p}] response start`, ee({
      retryOfRequestLogID: o,
      url: y.url,
      status: y.status,
      headers: y.headers,
...
class _c {
  ...
  async makeStreamingRequest(e) {
    ...
    try {
      for await (const a of n) {
        o = a;
        const u = a.text;
        if (u && this.tokenCallback && this.tokenCallback(u), (r = (i = a.candidates) == null ? void 0 : i[0]) == null ? void 0 : r.finishReason)
          break;
      }
    } catch (a) {
  ...
  extractTextContent(e) {
    ...
    try {
      ...
      return console.error("[GeminiToolHandler] Error extracting text content", {
        error: r,
        errorMessage: r instanceof Error ? r.message : String(r)
      }), null;
    }
          ...
          candidates: (u = m.candidates) == null ? void 0 : u.map((_) => {
            ...
            return {
              ...
              content: {
                ...
                partsCount: (E = (S = _.content) == null ? void 0 : S.parts) == null ? void 0 : E.length,
                partTypes: (A = (C = _.content) == null ? void 0 : C.parts) == null ? void 0 : A.map((R) => R.text ? "text" : R.inlineData ? `inlineData(${R.inlineData.mimeType})` : R.functionCall ? `functionCall(${R.functionCall.name})` : R.functionResponse ? `functionResponse(${R.functionResponse.name})` : "unknown")
              }
            };
          }),
...
const ne = class ne {
  ...
  logChatAudit(e) {
    ...
    if (this.config.logToConsole) {
      ...
      console.log(`[AUDIT] Chat completed: ${i.provider}/${i.model} (${i.totalDuration}ms)`, JSON.stringify(i));
    }
    this.config.logToServer && this.config.serverEndpoint && this.sendToServer(n, this.config.serverEndpoint);
  }
  /**
